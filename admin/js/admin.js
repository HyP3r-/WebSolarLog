 ...
 */
function init_menu() {
    $(".btnAdvanced").bind('click', function () {
        init_advanced();
    });
    $(".btnGeneral").bind('click', function () {
        init_general();
    });
    $(".btnDevices").bind('click', function () {
        init_devices();
    });
    $(".btnGraphs").bind('click', function () {
        init_graphs();
    });
    $(".btnCommunication").bind('click', function () {
        init_communication();
    });
    ...
    $(".btnYields").bind('click', function () {
        init_yields();
    });
    $(".btnDiagnostics").bind('click', function () {
        init_diagnostics();
    });
    $(".btnTariff").bind('click', function () {
        init_tariff();
    });
    $(".btnSocial").bind('click', function () {
        init_social();
    });
    ...
    $(".btnPlugwise").bind('click', function () {
        init_plugwise();
    });
    $(".btnDataMaintenance").bind('click', function () {
        init_dataMaintenance();
    });
}
        ...
        $.ajax({
            ...
            success: function (source) {
                ...
                $("div.switchPlug").click(function () {
                    ...
                    $('#' + id).html('Switching....');
                    $.get('admin-server.php?s=switchPowerState', {
            ...
            $.getJSON('admin-server.php?s=dropboxGetFiles', function (data) {
                $.ajax({
                    ...
                    success: function (source) {
                        ...
                        $('#makeBackup').bind('click', function () {
                            makeBackup();
                        });
                        $('#dropboxSync').bind('click', function () {
                            dropboxSync();
                        });
                        $('#detachDropbox').bind('click', function () {
                            detachDropbox();
                        });
...
function load_device(deviceId, deviceApi, deviceType) {
    ...
    WSL.connect.getJSON('admin-server.php?s=inverter&id=' + deviceId, function (inv_data) {
        ...
        if (inv_data.inverter.id > 0 && inv_data.inverter.panels.length == 0 && inv_data.inverter.type == 'production') {
            $.pnotify({
                title: 'No System Panels',
                text: 'Please add one or more panels. We need panels for some calculations.'
            });
            ...
            $.ajax({
                ...
                success: function (source) {
                    ...
                    $("input[name = 'chkNewTrunk']").bind('click', function () {
                        ...
                        $.post('admin-server.php', {
                            's': 'save-checkNewTrunk',
                            chkNewTrunk: checkNewTrunk
                        }, function (result) {
