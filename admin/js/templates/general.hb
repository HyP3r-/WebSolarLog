<form>
    ...
    <fieldset>
        ...
        <label for="longitude">longitude:</label><input type="text" name="longitude" value="{{data.longitude}}"/><a
            href="#" id="btnSetLatLong">}Set coordinates</a><br/>
        ...
        <select name="gaugeMaxType">
            <option value="panels" {{#if_eq data.gaugeMaxType compare="panels" }}selected=selected{{/if_eq}}>
                Static(panels)
            </option>
            <option value="average" {{#if_eq data.gaugeMaxType compare="average" }}selected=selected{{/if_eq}}>
                Dynamic(avg. power)
            </option>
        </select><br/>
        ...
        <select name="timezone">
            {{#each data.timezones}}
            <option value="{{this}}" {{#if_eq ..
            /data.timezone compare=this}}selected=selected{{/if_eq}}>{{this}}</option>
            {{/each}}
        </select><br/>
        <label for="frontLivePoll">Frontend live poll to database:</label>
        <input type="text" size="5" name="frontendLiveInterval" id="frontendLiveInterval"
               value="{{data.frontendLiveInterval}}"/>seconds
        <div id="sliderFrontendLiveInterval" class="span-28"></div>
        <br>
