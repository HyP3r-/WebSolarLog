...
<form>
    <fieldset>
        ...
        <a href="https://groups.google.com/forum/#!forum/websolarlog" target="_blank">Visit our WebSolarLog
            support-group</a><br>
        <br>
...
<form>
    <fieldset>
        ...
        <font style="color:#FFCC00">WebSolarLog is running.</font><br/>Please run the following command on the prompt of
        your Linux system to start WebSolarLog:<br/>
        ...
        {{else}}
        <font style="color:#FFCC00">WebSolarLog is not running.</font><br/>Please run the following command on the
        prompt of your Linux system to start WebSolarLog:<br/>
...
<form>
    <fieldset>
        ...
        <label>DB changed(<{{data.test.db.margin}} sec):</label>{{#if data.test.db.dbChanged}}Within the
        {{data.test.db.margin}} sec.{{else}}Longer then {{data.test.db.margin}} sec. ago.{{/if}}<br/>
        ...
        <font style="color:#4B8902">The Owner needs atleast Read and Write (>=6) permissions.<br>It looks like the
            webserver user has sufficient rights on the database.</font>
        {{else}}
        <font style="color:#FF0000">It looks like the webserver user has <strong>insufficient rights</strong> on the
            database</font>
        {{/if_gteq}}
        <div>
            {{else}}
...
<form>
    <fieldset>
        ...
        <div id="extensions">
            <div>
                ...
                <strong>
                    ...
                    Exists
                    {{/if_eq}}
                </strong><br><font style="color:#FF0000">It looks like the SQLite DB plugins are mixed.<br>We need
                SQLite3 and your system got SQLite.</font>
                {{else}}
                <strong>
                    {{#if_eq this.type compare="extension"}}
                    Loaded
        ...
        <div id="sqlite">
            ...
            {{else}}
            <font style="color:#FF0000">Your SQLite version is to old and you may experiance unexpected behaviour.<br>Please
                update your SQLite version to atleast 3.7.11</font>
            {{/if}}
            <br><Br>
        </div>
        <div id="sqlite">
            ...
            <font style="color:#4B8902">This is good.</font>
            {{else}}
            <font style="color:#FF0000">Your PHP version is to old and you may experiance unexpected behaviour.<br>Please
                update your SQLite version to atleast 3.7.11</font>
            {{/if}}
        </div>
    </fieldset>
