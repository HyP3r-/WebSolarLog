    ...
    <div class="column span">
        <div class="span-48">
            ...
            <div class="span-14 first summaryblock">
                ...
                Total household CO2 usage equals the daily consumption of <span
                    title="The household produces {{data.total.householdCO2}} CO2 and a tree consumses {{data.total.co2CompensationTree}} grams of CO2 a day so we need {{data.total.totalUsageCO2Trees}} trees to compensate this"><strong>{{data.total.householdTrees}}</strong></span>
                trees.
            </div>

            <div class="span-14 summaryblock">
                <strong>Household usage data:</strong><br>
                from grid: {{data.total.householdUsage}} kWh, {{data.total.householdCO2}} kg,
                {{data.total.householdCosts}}<br>
                from sun: {{data.total.usedBeforeMeterKWH}} kWh, {{data.total.usedBeforeCO2}} kg,
                {{data.total.usedBeforeMeterCosts}}<br>
                household: <span
                    title="We generated {{data.total.production.KWH}} of that we used {{data.total.usedBeforeMeterKWH}} in the house, so we returned {{data.total.metering.returnKWH}} to the grid">{{data.total.totalUsagekWh}} kWh</span><br>
                Money{{#if_gt data.total.metering.gasUsageCO2 compare=0}}combined with
                {{data.total.metering.gasUsageCO2}} CO2 for gas {{/if_gt}}gives a total of {{data.total.householdCO2}}
                for the household">{{data.total.householdCO2}}</span></strong> kg CO2 and
            ...
            <div class="span-14 last summaryblock">
                ...
                The returned kWh's equals <span title="Somebody else is getting your carbon free power :D"><strong>{{data.total.metering.returnCO2}}</strong> kg CO2 and <span
                    title="We returned this to the grid, so somebody else can use it."> and {{data.total.moneySign}} {{data.total.metering.returnCosts}}</span>
				{{else}}
